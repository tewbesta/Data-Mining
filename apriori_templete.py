...
def apriori(dataset, min_support=0.5, verbose=False):
    ...
    F.sort()
    print("L1 frequent items are {} which are {}".format(sum( [ len(listElem) for listElem in F]), F ) )
    while (len(F[k - 2]) > 0):
        Ck = apriori_gen(F[k-2], k) # generate candidate itemsets
        print("C {} candidate items are {} ".format(k, len(Ck)))
        Fk, supK  = get_freq(D, Ck, min_support) # get frequent itemsets
        print("L {} frequent items are {} which are {}".format(k, len(Fk), Fk))
...
def apriori_gen(freq_sets, k):
    ...
    lenfs = len(freq_sets)  # number of frequent itemsets
    for i in range(lenfs):
        for j in range(i + 1, lenfs):
            a = list(set(freq_sets[i]))
            b = list(set(freq_sets[j]))
            a.sort()
            b.sort()
            ...
            if F1 == F2:  # if the first k-2 items are identical
                ...
                retList.append((freq_sets[i]).union(freq_sets[j]))
    retList.sort()
    return set(retList)
